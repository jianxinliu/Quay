...
- MySQL / PostgreSQL / SQLite，SSH 多跳隧道直达内网库
  （**Redis 有意不暴露给 agent**，仅供人通过后台 Redis 控制台操作）
...
- **Redis 控制台**：对标 Medis 的独立页——库（只列有数据的）→键（前缀树 + 类型徽章）、
  键详情（TTL/内存/编码）、命令窗口（Monaco 光标行执行，读直通/写确认）、命令文档面板
