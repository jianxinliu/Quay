...
- **合成打字对 CodeMirror 不可靠**：浏览器自动化 `type` 往 CodeMirror 里输入会掉字符；e2e 验证时用 `javascript_tool` 直接 `cm.CodeMirror.setValue(...)` 设值。另：`run(false)` 会先把结果区置「执行中…」再弹确认模态，模态浮在上层——点击脱靶关掉模态会露出遗留占位，看着像「卡住」，实际请求未发（已在 showConfirm 里清空结果区修掉）。
...
- `export.py` 查询结果导出纯函数（CSV/JSON/Markdown/xlsx；openpyxl 惰性导入）；`static/` vendor 的 CodeMirror 5（由 admin `/admin/static/{name}` 路由服务，需 keyring 之外无新前端依赖）
- **查询台**（DataGrip 风格，admin.py `/admin/sql*`）：元信息浏览 + CodeMirror 编辑器（高亮/格式化/保存.sql）+ 结果表格 + 多格式导出。**后台专属写入路径 `service.admin_run_sql(confirm=)`**：读直跑 reader；写未确认返回风险报告、确认后用 writer **直接执行**并审计（tool=`admin_execute`），**不进审批单**——这是后台旁路，「拒绝—重提」红线只约束 agent 的 execute。导出仅限只读语句。所有 DB 触达的路由用 `anyio.to_thread.run_sync` 卸载，避免阻塞同 ASGI 上的 agent 请求
...
- [x] M6 查询台：元信息浏览 + SQL 编辑器（CodeMirror 高亮/格式化/保存）+ 结果表格 + CSV/JSON/Markdown/xlsx 导出；写操作二次确认后 writer 直接执行（后台旁路，不进审批单）。真浏览器 e2e 验证：读查询、写确认弹窗+执行、导出 xlsx 均通过。200 个测试全过。导入**有意未做**（批量写风险面大，下一期专门设计）
