...
- **查询台 SQL 串行执行 + 计时 + 取消（`jobs.py::JobManager`）**：后台异步查询原本每个 job 起一个 daemon 线程、无序并发。改为**按 queue_key 串行**（同一连接同时只跑一条 SQL、其余 FIFO 排队；不同连接/workflow 各自并行——workflow/画布用 `object()` 唯一 key 不参与串行）。worker 线程惰性建、**队列排空即在锁内注销退出**（避免每连接常驻空转线程）。**取消**：排队中直接标记 canceled、worker 取到时跳过；运行中调用 `engines.make_canceller`——MySQL `KILL QUERY <thread_id>`、PG `pg_cancel_backend(pid)`、SQLite `interrupt()`，在**另开连接**上发（SQLite 例外，对同一连接 interrupt）。取消器经 `run_query/run_write` 的 `on_start` 回调注册。前端 `/admin/sql/job` 轮询带 `status(queued/running)/queue_position/elapsed_ms`，编辑器顶部浮「执行中 Ns / 排队中（前面 N 条）+ 取消」pill。真实 MySQL e2e + 浏览器 e2e 全过（pill 计时、取消→已取消、同连接第二条排队）。
...
- `jobs.py` 后台查询台异步任务管理器（`JobManager`）：按 queue_key 串行 + 排队位置 + 计时 + 取消（DB 层 KILL）；纯逻辑可单测（test_jobs.py）。admin 的 `/admin/sql/{run_async,job,cancel}`、workflow/画布 run 都走它
...
- [x] 查询台 SQL 串行执行 + 读写超时可配 + 计时/取消（分支 `feat/sql-serial-exec`）：① **根治大 DELETE 报 2013**——writer 独立 `write_timeout_s`（默认 600s，连接表单可配读/写超时），reader 保持 30s（详见「经验教训」）② **按连接串行**（`jobs.py::JobManager`，同连接一次一条 + 排队、异连接并行）③ **计时 + 取消**——编辑器顶部 pill 显「执行中 Ns / 排队中」+ 取消按钮，运行中取消对 DB 发 `KILL QUERY`/`pg_cancel_backend`。仅约束人用的查询台（agent MCP 路径不变）。304 测试全过（+test_jobs 11、engines 角色超时/make_canceller、admin 异步 job 字段/取消）；真实 MySQL e2e（`scripts/e2e_serial_cancel.py`）+ 浏览器 e2e（pill 计时 4.1s、取消→已取消、同连接第二条排队 queue_position=1）全过
