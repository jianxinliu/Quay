...
- [x] P2 workflow：provenance 自动记取数配方（工作区 __provenance 表）+ WorkflowStore（SQLite）+ workflow_run 重拉数据→逐步执行→输出预览（任一步失败即停）；查询台「存工作流」+ 左栏工作流区（▶重跑/载入/删）；MCP run_workflow 开放。P3 可视化待做
