...
| 只读查询 | `query` | 仅 SELECT/SHOW/DESCRIBE/EXPLAIN；默认注入 LIMIT 与超时 |
| 数据变更 | `execute` + `get_change_status` | 拒绝—重提审批流，见下 |
| 连通性 | `test_connection` | SELECT 1 |
| Redis | `redis_command` | 读命令直通；危险命令走同一套审批 |
| 跨源分析 | `analysis_workspaces` / `analysis_import` / `analysis_sql` | DuckDB 本地沙箱，见下 |
| 流程重跑 | `run_workflow` | 一键重跑人/agent 沉淀的分析流程 |
...
- 你**不能创建/修改/删除 workflow**（当前版本）；想沉淀分析时，把脚本给用户，
  请他在查询台「存工作流」。
